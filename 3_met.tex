...
Com base nas estatísticas dos intervalos R-R, o algoritmo classificou o segmento em cinco categorias de anormalidade: taquicardia, bradicardia, arritmia sinusal, bloqueio sinoatrial e parada sinoatrial. A taquicardia foi sinalizada quando o intervalo R-R médio foi inferior a 600 ms (frequência cardíaca acima de 100 bpm) e a bradicardia quando superior a 1000 ms (abaixo de 60 bpm). A arritmia sinusal foi sinalizada quando a diferença entre dois intervalos R-R consecutivos excedeu 120 ms.
...
Registre-se que esta formulação relativa constitui a única alteração de regra de decisão introduzida no algoritmo reaproveitado de \citet{moraesFirmwareECG}. Em sua versão original, ambas as pausas eram decididas por duração absoluta --- bloqueio entre 4 e 6~s, parada acima de 6~s ---, faixa que corresponde a pausas de gravidade bem superior à do quadro que se pretende reconhecer: a uma frequência cardíaca usual, um bloqueio de dois a três ciclos produz pausa de aproximadamente 1,6 a 2,4~s e não alcançaria o piso de 4~s. A substituição foi conduzida sob orientação dos autores do algoritmo e sua consequência experimental é reportada na Seção~\ref{sec:resValidacaoFinal}; as demais regras --- limiares de taquicardia, bradicardia e arritmia sinusal, contadores e precedências --- permaneceram como no original.
...
\label{alg:classif}
\KwIn{Picos R $\mathcal{R}$; limiares $RR_{taq}=600$ ms, $RR_{bra}=1000$ ms e tolerância $\tau=0{,}05$}
...
\For{$i \leftarrow 0$ \KwTo $|RR|-2$}{
  ...
  \lIf{$\left|\rho/2-1\right| \le \tau$ \textbf{ou} $\left|\rho/3-1\right| \le \tau$}{$b \leftarrow b+1$}
  \ElseIf{$\rho > 3(1+\tau)$}{
