...
A Figura~\ref{fig:topologia} apresenta a arquitetura completa do sistema. O sinal foi captado por eletrodos conectados ao paciente, digitalizado, processado e transmitido por um istema embarcado portátil, montado experimentalmente. O \textit{gateway} \textit{LoRaWAN} encaminhou os pacotes ao servidor de rede \textit{ChirpStack}, que os publicou em um \textit{broker} \acs{MQTT}. Um serviço de ingestão consumiu essas mensagens e gravou os eventos no banco de dados em nuvem (\textit{Firestore}), de onde o aplicativo móvel os leu em tempo real.
...
\section{Dispositivo de Aquisição e Procesamento do Sinal de ECG}
...
A arritmia sinusal é sinalizada quando a diferença entre dois intervalos R-R consecutivos situa-se entre 120 ms e 80\,\% do intervalo R-R de base. O limite superior era utilizado no algoritmo original original e foi acrescentado por uma razão específica: a pausa característica de um bloqueio sinoatrial produz, ela própria, uma diferença entre intervalos sucessivos muito maior que 120 ms --- um bloqueio de um batimento gera diferença igual a um intervalo R-R inteiro ---, de modo que o critério de arritmia era atendido pelo bloqueio também, gerando classificação dupla. Sem o teto, era necessário suprimir a detecção de arritmia sempre que houvesse detecção de bloqueio também. Intervalos que escapam a essa faixa e à do critério de bloqueio sinoatrial, descrito a seguir, são tratados pela sexta categoria de anormalidade, introduzida na Seção~\ref{sec:resNaoCaracterizada} para cobrir exatamente essa lacuna (Seção~\ref{sec:resZonaMorta}).

As duas classes de pausa foram decididas em relação ao ritmo de base do próprio segmento, e não por duração absoluta, em conformidade com sua definição clínica: no bloqueio sinoatrial de segundo grau o nó sinusal despolariza mas o impulso é barrado antes de atingir o átrio, de modo que a pausa corresponde a um múltiplo inteiro do intervalo entre batimentos — dois ou três intervalos R-R —, ao passo que na parada sinusal o nó deixa de disparar e a pausa não guarda proporção inteira com o ritmo de base, excedendo três ciclos. Adotou-se como ritmo de base a mediana dos intervalos R-R do segmento, e não a média: por ser uma estatística de ordem, a mediana não é deslocada pelas próprias pausas que se pretende medir, ao passo que a média seria puxada para cima por elas, reduzindo a proporção entre o intervalo pausado e o ritmo de base e ocultando a própria pausa que o critério deveria capturar. Como a igualdade exata com o múltiplo é inatingível na presença da variabilidade fisiológica e da quantização temporal imposta pela taxa de amostragem, a comparação admitiu uma tolerância relativa de 5\,\%. Não se impôs qualquer piso de duração absoluta: a uma frequência cardíaca elevada, uma pausa de dois ciclos é breve em termos absolutos, mas continua sendo, por definição, um bloqueio.
