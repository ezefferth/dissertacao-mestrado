...
Para cada uma das seis classes — normal, taquicardia, bradicardia, arritmia sinusal, bloqueio sinoatrial e parada sinoatrial — selecionou-se uma forma de onda representativa, com classe e frequência cardíaca conhecidas (gabarito): as classes de ritmo derivaram de registros de \acs{ECG} reais, ao passo que as classes de pausa (bloqueio e parada sinoatrial), escassas nas bases públicas, foram reproduzidas a partir de sinais sintéticos contendo as pausas características. Cada forma de onda, com duração de 30~s, foi reproduzida de modo contínuo no gerador, e o \textit{firmware} processou 30 segmentos consecutivos de 30~s de cada classe, totalizando 180 ensaios. A taxa de reprodução do gerador foi fixada em 640 amostras por segundo, igual à taxa efetiva do conversor do dispositivo, evitando o descasamento de amostragem entre gerador e \acs{AFE}. Para cada segmento, a classe detectada pelo \textit{firmware} e o evento efetivamente entregue ao aplicativo foram confrontados com o gabarito, permitindo quantificar tanto o \textbf{processamento} (a anomalia foi corretamente identificada?) quanto a \textbf{transmissão} (o evento correto chegou ao aplicativo?). O desempenho foi expresso por meio de uma matriz de confusão e das métricas de sensibilidade, especificidade, valores preditivos positivo e negativo e acurácia por classe, além do erro de estimativa da frequência cardíaca, em conformidade com o protocolo do trabalho que fundamentou o algoritmo de detecção (Seção~\ref{sec:deteccaoBorda}).
