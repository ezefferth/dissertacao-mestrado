...
Esse \textit{firmware} constitui a base do algoritmo de detecção embarcada adotado neste trabalho: seu código-fonte foi cedido pelos autores e reutilizado, não constituindo, portanto, contribuição original deste trabalho. A opção pelo reúso, em vez da reimplementação do algoritmo detector, decorre código já ter sido validado e de o foco desta pesquisa recair sobre a aquisição, o processamento na borda e a transmissão ddos eventos de alerta via \textit{LoRaWAN}. Enquanto o \textit{firmware} original foi concebido para um monitor multiparâmetros com geração de alertas do próprio equipamento, o presente trabalho parte do mesmo algoritmo detector e o integra a uma arquitetura de transmissão de longo alcance por \textit{LoRaWAN}, com o posterior encaminhamento dos eventos para a nuvem e para um aplicativo móvel — aspecto não contemplado no trabalho de origem, e que constitui uma das contribuições desta pesquisa. Os detalhes da adaptação realizada nsete \textit{firmware} para a plataforma adotada neste trabalho são apresentados na Seção~\ref{sec:deteccaoBorda}.
