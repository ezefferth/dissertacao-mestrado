...
%O monitoramento remoto de sinais eletrocardiográficos (\acs{ECG}) tem se tornado cada vez mais relevante em aplicações clínicas e domiciliares, exigindo tecnologias de comunicação sem fio que conciliem eficiência energética, confiabilidade e capacidade de transmissão contínua de dados. A escolha da tecnologia adequada depende de fatores como taxa de transmissão, alcance, consumo de energia e compatibilidade com dispositivos móveis.
...
% As principais características técnicas do protocolo LoRaWAN incluem a operação em bandas de frequência não licenciadas, com variações conforme a região geográfica: 868 MHz na Europa, 915 MHz nas Américas e 433 MHz em partes da Ásia (\citeauthor{s23208440}, \citeyear{s23208440}). O tamanho máximo do payload transmitido é condicionado à configuração do fator de espalhamento (spreading factor) e da largura de banda, sendo que a sobrecarga de cabeçalho típica gira em torno de 14 bytes. Além disso, dispositivos de Classe A apresentam consumo de energia extremamente baixo, com corrente de repouso inferior a 2 µA, o que viabiliza uma autonomia de bateria de até 10 anos, tornando-os ideais para aplicações remotas e de longa duração (Wio-SX1262 Datasheet, 2024). Destaca-se ainda a elevada imunidade a interferências eletromagnéticas e a robustez frente a efeitos de propagação multipercurso (multipath), proporcionadas pelo uso da modulação por espalhamento espectral do tipo chirp spread spectrum (CSS).
...
%Outra tecnologia emergente que tem ganhado atenção no contexto de monitoramento remoto de ECG é o LoRa (\textit{Long Range}), baseada em modulação por espalhamento espectral (\textit{chirp spread spectrum}). Diferente das tecnologias citadas anteriormente, o LoRa prioriza alcance e eficiência energética em detrimento da taxa de dados, operando com \textit{throughput} típico na ordem de alguns kbps. Além disso, seu alcance de vários quilômetros (5 km zonas urbanas e 15 km zonas ruais) e operação em bandas sub-GHz o tornam ideal para zonas rurais e comunidades remotas com infraestrutura limitada. A arquitetura do tipo star-of-stars permite que múltiplos dispositivos se comuniquem com um único gateway, reduzindo o custo da infraestrutura de rede (\citeay{lorawan1}). LoRaWAn possui algumas características como frequência de banda que varia em continentes, Max. payload size, largura de banda, data rate
