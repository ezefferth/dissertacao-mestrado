...
O trabalho de \cite{7814958} emprega a \acs{IoT} como meio de transmissão dos dados do sinal de \acs{ECG}, propondo um sistema de baixo custo para o monitoramento de múltiplos pacientes, acessível simultaneamente por vários usuários pela internet. O sistema consiste em um circuito analógico dedicado à captura do \acs{ECG}, com eletrodos posicionados conforme o triângulo de \textit{Einthoven}, um amplificador de biopotencial, um filtro passa-alta de primeira ordem (0,05 Hz), para atenuar o artefato de movimento, e um filtro \textit{Butterworth} passa-baixa de terceira ordem (40 Hz). O microcontrolador ATmega328 digitalizou o sinal e o transmitiu serialmente ao servidor por meio de módulos \textit{XBee Series 2} (\textit{ZigBee}, IEEE 802.15.4), um por aparelho, operando como \textit{end devices} de um coordenador acoplado ao servidor; cada quadro é precedido de um caractere identificador do paciente, o que permite separar as fontes na reconstrução. Os sinais são armazenados no servidor e disponibilizados em uma aplicação web com três níveis de acesso --- administrador, médico e paciente ---, incluindo troca de mensagens entre médico e paciente. A avaliação relatada concentrou-se na capacidade do servidor: por meio de ferramentas de teste de carga, simulou-se o acesso concorrente de 10 a 150 usuários à aplicação, atendendo-se até 20 acessos simultâneos sem erro, ao passo que, de 50 a 150 usuários, surgiram falhas atribuídas ao tráfego elevado e à largura de banda insuficiente. Não foi reportada, contudo, a aquisição simultânea de \acs{ECG} nessa mesma escala.

O estudo de \cite{9920379} propõe uma solução inovadora para o monitoramento remoto em tempo real do status cardíaco de pacientes, utilizando sensores ECG e comunicação via LoRa 915 MHz em uma rede \acs{IoT}. O ECG, essencial para detectar condições cardíacas como arritmias, é transmitido por meio da tecnologia LoRa. Os dados do ECG são recebidos pela plataforma \textit{ThingSpeak}, permitindo que médicos e pacientes monitorem a saúde cardíaca em tempo real, independentemente da localização. O estudo também avaliou o desempenho do LoRa em condições de sinal variável, especialmente em áreas densamente povoadas, analisando parâmetros como perda de pacotes, intensidade de sinal e taxa de erro. Os resultados demonstram a viabilidade e a eficácia da tecnologia para o monitoramento remoto e acessível da saúde cardíaca. Para a implementação, foram utilizados o módulo LoRa 915 MHz, o Dragino LoRa 915 MHz Gateway, o Arduino Uno e o módulo ECG AD8232.

No trabalho de \cite{8934622}, foi implementado um protótipo de monitoramento de ECG baseado em \acs{IoT}, que se destacou como uma solução eficiente e de baixo custo, apresentando uma precisão de 80\% nas capturas dos sinais. O dispositivo utiliza a tecnologia de comunicação Wi-Fi, aliado a uma placa microcontroladora Arduino UNO para a captura dos sinais de ECG, com o módulo AD8232 conectado a ela. O Raspberry Pi foi empregado como plataforma de processamento dos dados, permitindo a análise em tempo real dos sinais de ECG. Além disso, foi desenvolvida uma interface gráfica acessível via web e aplicativos móveis, proporcionando aos usuários a visualização e monitoramento contínuo dos dados de saúde do paciente.
...
Em síntese, os trabalhos analisados convergem quanto à viabilidade técnica do monitoramento remoto de \acs{ECG} baseado em \acs{IoT}, mas evidenciam um compromisso recorrente entre a tecnologia de comunicação adotada e os requisitos de alcance, consumo energético e taxa de dados. Enquanto soluções baseadas em Wi-Fi e ZigBee privilegiam a taxa de transmissão em detrimento do alcance e da autonomia, as abordagens emergentes baseadas em LoRa buscam justamente o oposto, abrindo espaço para o monitoramento em áreas extensas e com infraestrutura limitada, contexto no qual se insere este trabalho.



% TODO: se pertinente, citar Yeh e Wang (método DOM original) e outros trabalhos
% correlatos de detecção embarcada que o usuário fornecer.
% (Dados de publicação de moraesFirmwareECG já confirmados: SEMISH 2026, p. 251-262.)
...
Entre as tecnologias emergentes voltadas para a \acs{IoT}, destaca-se o LoRa (\textit{Long Range}), especialmente no contexto do monitoramento remoto de sinais \acs{ECG}. Trata-se de uma tecnologia de comunicação sem fio que emprega modulação por espalhamento espectral do tipo \textit{chirp spread spectrum} (CSS), permitindo a transmissão de dados a longas distâncias com baixo consumo de energia (\citeay{lorawan1}). 
...
Além da morfologia das ondas, a sequência dos intervalos R-R --- o tempo decorrido entre batimentos consecutivos --- constitui uma segunda dimensão de informação diagnóstica, obtenível a partir de uma única derivação. O valor médio dessa série define a frequência cardíaca e, com ela, a classificação do ritmo em taquicardia, acima de 100~bpm, ou bradicardia, abaixo de 60~bpm, em repouso (\citeay{brazilian_guidelines_ecg}; \citeay{guyton2021fisiologia}). Já a dispersão e o padrão de irregularidade dessa mesma série caracterizam um conjunto de alterações do ritmo: a variação cíclica acompanhando a respiração, na arritmia sinusal; a irregularidade sustentada e sem padrão repetitivo, típica da fibrilação atrial; a antecipação do batimento seguida de pausa compensatória, produzida pelas extrassístoles; e o prolongamento abrupto do intervalo, indicativo de pausas sinusais e de bloqueios atrioventriculares avançados (\citeay{goldberger_ecg2006}; \citeay{marriott2018practical}). Por serem intermitentes, muitas dessas alterações escapam ao registro convencional de repouso e só se evidenciam sob monitoramento prolongado, como evidenciado pela eletrocardiografia dinâmica em idosos assintomáticos (\citeay{arritmiasIdosos}).

Essa distinção delimita o escopo do presente trabalho. A caracterização topográfica de eventos isquêmicos e de distúrbios de condução depende da comparação entre as 12 derivações e, portanto, da disponibilidade da forma de onda completa. A detecção das alterações de ritmo descritas acima, por sua vez, depende apenas da localização dos complexos QRS ao longo do tempo, podendo ser reduzida a um pequeno conjunto de descritores --- frequência cardíaca, intervalo R-R médio, sua dispersão e o número de complexos detectados no segmento --- extraídos no próprio dispositivo (\citeay{moraesFirmwareECG}). É essa redução que viabiliza o monitoramento sobre uma rede de baixa taxa de dados, estratégia detalhada no Capítulo~\ref{chap:metodologia}.
