...
Optou-se por relatar a progressão na íntegra, e não apenas o resultado final, por duas razões. A primeira é que essa etapa documenta a metodologia de depuração empregada e permite distinguir as limitações do \textit{firmware}, das limitações intrínsecas do detector ou do banco de estímulos — distinção que um único número de exatidão não revelaria. A segunda é que três das correções mais importantes do trabalho — a perda de amostras na aquisição, a descontinuidade do intervalo R-R na junção entre o fim e o início do arquivo reproduzido em laço pelo gerador (adiante designada \emph{emenda do laço}) e o critério de pausa — só se tornaram visíveis por comparação entre baterias, e omitir as baterias que falharam suprimiria justamente a evidência que sustenta as correções. A Tabela~\ref{tab:progressaoBaterias} e a Figura~\ref{fig:progressaoBaterias} sintetizam as baterias conduzidas.
