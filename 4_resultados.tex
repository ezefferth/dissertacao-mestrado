...
A caracterização quantitativa do desempenho de detecção foi obtida pelo método de injeção de sinais de classe conhecida descrito na Seção~\ref{sec:validacaoGerador}. Para cada uma das seis classes — normal, taquicardia, bradicardia, arritmia sinusal, bloqueio sinoatrial e parada sinoatrial —, reproduziu-se de forma contínua, na saída do gerador de sinais, uma forma de onda representativa da classe, da qual o \textit{firmware} processou 30 segmentos consecutivos de 30~s, totalizando 180 ensaios por bateria. A classe atribuída a cada segmento resultou do colapso da máscara de anomalias segundo a ordem de precedência adotada neste trabalho (Seção~\ref{sec:resPipeline}). Convém distinguir, para a leitura das matrizes de confusão, as seis \emph{classes de estímulo} --- que incluem a condição normal --- das seis \emph{categorias de anormalidade} que o detector sinaliza, entre as quais a anomalia não caracterizada, acrescentada ao cabo do percurso (Seção~\ref{sec:resNaoCaracterizada}). As matrizes admitem, portanto, sete colunas de saída: as seis classes e a categoria adicional, que não corresponde a nenhum estímulo do banco e cuja ocorrência indica que o segmento não foi caracterizável.

O objetivo desta validação é verificar o comportamento desse detector sobre a cadeia de aquisição e de transmissão desenvolvida nesta dissertação. A validação não se deu em um único experimento, mas em uma sucessão de baterias, e convém explicitar desde já como o presente capítulo as organiza. As baterias relatadas nesta seção e nas duas seguintes constituem a etapa de depuração: cada uma expôs uma limitação distinta — ora do banco de estímulos, ora da cadeia de aquisição, ora do próprio critério de detecção —, cuja correção orientou a bateria seguinte. Nenhuma delas mede o sistema em sua configuração definitiva, e seus valores de exatidão não devem ser lidos como o desempenho do sistema. Esse desempenho é o da validação final (Seção~\ref{sec:resValidacaoFinal}), conduzida sobre a configuração de \textit{firmware} efetivamente entregue e sobre o banco de estímulos consolidado na etapa de depuração.

Optou-se por relatar a progressão na íntegra, e não apenas o resultado final, por duas razões. A primeira é que essa etapa documenta a metodologia de depuração empregada e permite distinguir as limitações do \textit{firmware}, das limitações intrínsecas do detector ou do banco de estímulos — distinção que um único número de exatidão não revelaria. A segunda é que três das correções mais importantes do trabalho — a perda de amostras na aquisição, a \emph{descontinuidade entre segmentos} --- o intervalo R-R espúrio que surge na junção entre o fim e o início do arquivo reproduzido em laço pelo gerador --- e o critério de pausa — só se tornaram visíveis por comparação entre baterias, e omitir as baterias que falharam suprimiria justamente a evidência que permitiu as correções. A Tabela~\ref{tab:progressaoBaterias} sintetiza as baterias conduzidas.
...
\footnotesize
\begin{tabular}{c >{\raggedright\arraybackslash}p{3.3cm} >{\raggedright\arraybackslash}p{3.3cm} >{\raggedright\arraybackslash}p{1.6cm} >{\raggedright\arraybackslash}p{3.1cm}}
\hline
\textbf{Bat.} & \textbf{Morfologia do batimento} & \textbf{Frequência cardíaca} & \textbf{Taxa de acerto total (\%)} & \textbf{Limitação exposta} \\
\hline
1 & Banco externo, reproduzido sem alteração (sintético, batimento invariante) & Taquicardia a 101~bpm, sobre o limiar & 67,8 & Especificidade da arritmia; fronteira do limiar \\ [2pt]
2 & Banco externo, estímulos de fronteira substituídos & Taquicardia a 120~bpm, afastada do limiar & 78,3 & Perda de amostras na aquisição \\ [2pt]
3 & Sintética: batimento por soma de gaussianas, amplitude constante & Distante dos limiares; R-R múltiplo do período de amostragem & 100,0 & (limite superior otimista) \\ [2pt]
4 & Banco externo, reproduzido sem alteração & Normal e arritmia a exatamente 60~bpm, sobre o limiar de bradicardia & 60,6 / 65,0 & Descontinuidade entre segmentos \\ [2pt]
5 & Batimento do banco externo, com ritmo sintetizado e imperfeições de amplitude & Controlada e distante dos limiares, com \textit{jitter} do R-R & 100,0 & --- \\
\hline
\end{tabular}
\end{table}
...
\caption{Taxa de acerto total da detecção em cada bateria de validação. As baterias livres de artefatos experimentais (3 e 5, em destaque) atingiram 100\,\%, enquanto as baterias de diagnóstico (1, 2, 4 e 4b) expuseram, cada uma, uma limitação distinta do banco de estímulos ou da aquisição, anotada sob a respectiva barra. O contraste entre as baterias, e não o valor isolado de qualquer uma, é o resultado central.}
...
A primeira bateria empregou as seis formas de onda do banco externo, reproduzidas sem alteração, e resultou em taxa de acerto de 67,8\,\% (Tabela~\ref{tab:matrizInicial}). Os erros concentraram-se em três frentes. Convém registrar, desde já, que a atribuição de causas feita nesta etapa era provisória: a mais volumosa delas só seria corretamente explicada duas baterias adiante.

A primeira frente incidiu sobre o traçado normal: dos 30 segmentos, apenas 2 foram reconhecidos como normais e 23 foram sinalizados como arritmia. A causa foi então atribuída à baixa especificidade do critério de variabilidade R-R (Seção~\ref{sec:resArritmia}). A terceira bateria mostraria que a explicação era outra: a perda periódica de amostras na aquisição (Seção~\ref{sec:resPerdaAmostras}), que encurtava artificialmente parte dos intervalos R-R e fabricava a variabilidade que o critério media. Corrigida a aquisição, e sem alterar o estímulo, o reconhecimento do traçado normal subiu de 20\,\%, na segunda bateria, para 100\,\%.

A segunda frente foi a proximidade do limiar de decisão: o estímulo de taquicardia, a cerca de 101~bpm, corresponde a um intervalo R-R médio de aproximadamente 594~ms, a apenas 6~ms do limiar de 600~ms, de modo que pequenas variações da estimativa segmento a segmento cruzavam o limiar e suprimiam a classificação (22 de 30 detectados). A terceira decorreu do critério de bloqueio sinoatrial então vigente, que exigia mais de uma pausa de 4 a 6~s no mesmo segmento; com pausas esparsas, apenas 20 de 30 segmentos foram detectados.

Cabe descartar, ainda, uma hipótese que a discrepância entre a taxa nominal da biblioteca e a taxa efetiva do conversor (Seção~\ref{sec:resTaxa}) poderia sugerir: a de que a base de tempo do detector estivesse escalada. Não estava. O \textit{firmware} sempre operou com os 640~SPS medidos, e os 30 segmentos de bradicardia foram corretamente classificados nesta mesma bateria --- o que não ocorreria se todos os intervalos estivessem multiplicados por um fator. O que faltava não era a taxa correta, mas as amostras que deixavam de ser retidas nela.
...
A segunda bateria corrigiu os estímulos de fronteira: a taquicardia passou de 101 para 120~bpm (R-R de 500~ms, distante do limiar) e o critério de bloqueio foi ajustado. A taquicardia passou a ser detectada em todos os segmentos, elevando a taxa de acerto total para 78,3\,\%. O ECG normal, contudo, com taxa de acerto baixa, o que indicava uma causa ainda não identificada, independente do estímulo.

\subsection{Perda de amostras na aquisição}
\label{sec:resPerdaAmostras}

A persistência dos erros no traçado normal, mesmo após o ajuste dos estímulos, motivou duas hipóteses. A primeira atribuía o problema à frequência cardíaca do ECG normal exatamente igual ao limiar: o traçado normal de 60~bpm possui R-R de exatamente 1000~ms, coincidente com o limiar de bradicardia, com margem nula; a adoção de um estímulo de 75~bpm (R-R de 800~ms, 200~ms de margem) eliminou as marcações indevidas de bradicardia, mas não as de arritmia. A segunda hipótese atribuía o problema à escontinuidade entre segmentos de ECG gerados continuamente — como o arquivo tinha 30~s e o segmento de processamento também, ocorreria uma descontinuidade entre o final de um segmento gerado e o início do próximo; geraram-se então arquivos de 300~s, mas a hipótese foi refutada, pois 24 dos 30 segmentos continuaram classificadas erroneamente.

Esse resultado motivou o registro do sinal bruto adquirido. No traçado bruto (Figura~\ref{fig:perdaAmostras}), identificaram-se seis intervalos R-R anômalos, periódicos a cada aproximadamente 4,9~s, cada um com o intervalo de 800 reduzido para cerca de 710~ms. A causa foi localizada na aquisição, em um mecanismo distinto do problema relacionado à da interface serial descrita na Seção~\ref{sec:resFirmware}: agora com a saída de depuração já decimada, era a própria rotina de diagnóstico que consultava o espaço ocupado no sistema de arquivos local (\textit{LittleFS}) dentro do laço principal, a cada 5~s, bloqueando o processador por cerca de 105~ms; como a rotina de interrupção apenas sinaliza a disponibilidade de uma nova amostra, cerca de 66 amostras eram perdidas. O relatório de diagnóstico registrava 3.133 amostras válidas em vez das 3.200 esperadas — uma perda de 2,08\,\%, mais sutil que a da saturação serial (que reduzia a taxa a cerca de 560~SPS) e, por isso mesmo, mais difícil de identificar.

Ambos os problemas decorrem da mesma decisão de projeto: a rotina de interrupção associada ao sinal \acs{DRDY} apenas sinaliza a disponibilidade de uma nova amostra, e a leitura pelo barramento \acs{SPI} ocorre no laço principal. Nesse arranjo, a integridade da aquisição depende de o laço nunca bloquear por mais de um período de amostragem (1,56~ms a 640~SPS) --- condição silenciosamente violada pelas duas rotinas identificadas. O arranjo mais robusto é outro: executar a leitura do conversor dentro da própria rotina de interrupção, com prioridade máxima, gravando a amostra em memória antes de retornar; nesse caso, um bloqueio no laço atrasa o processamento, mas não descarta amostra alguma. Ele não foi adotado porque a leitura do \acs{AFE} é feita pela biblioteca de controle de terceiros empregada, cujas transações \acs{SPI} recorrem à interface padrão do \textit{framework} e não satisfazem os requisitos de execução em contexto de interrupção do ESP32, que exige que o código chamado resida na memória interna de instruções. Reescrever esse caminho implicaria substituir a biblioteca. Optou-se, em vez disso, por remover do laço as chamadas bloqueantes e por instrumentar a aquisição: o contador de interrupções \acs{DRDY} passou a ser comparado ao de leituras efetivadas, e a diferença entre ambos --- as amostras perdidas --- passou a constar do relatório de diagnóstico. Sob essa instrumentação, as baterias finais transcorreram sem perda registrada. A migração da leitura para a rotina de interrupção permanece como melhoria recomendada, por eliminar a falha por construção, e não por vigilância.
...
Esse problema explicava todos os sintomas observados: a estimativa de frequência cardíaca cerca de 2\,\% maior; a aparente taxa de amostragem de 627~SPS, quando a taxa efetiva de 640~SPS (Seção~\ref{sec:resTaxa}); e as marcações indevidas de arritmia, pois a diminuição periódica produzia variações R-R de 89 a 114~ms, muito próximas do limiar de 120~ms. A correção adequada consistiu em desacoplar a rotina de diagnóstico do laço de aquisição (Seção~\ref{sec:resFirmware}).

\subsection{Bateria com estímulo controlado}
\label{sec:resBateriaControlada}

Corrigida a aquisição, repetiu-se o ensaio com o mesmo estímulo, obtendo-se a classificação correta dos 180 segmentos. O traçado normal passou de 20\,\% para 100\,\% de taxa de acerto. Esse resultado, entretanto, deve ser lido com três ressalvas. Os estímulos eram sintéticos, de amplitude constante, e portanto não exercitavam o limiar global de amplitude do detector. Os intervalos R-R situavam-se distantes dos limiares de decisão, o que dispensava a estimativa de qualquer precisão. E cada intervalo R-R correspondia a um número inteiro de amostras --- 800~ms equivalem a exatamente 512 amostras a 640~SPS ---, de modo que a quantização do instante do pico não introduzia erro algum na estimativa da frequência cardíaca; com intervalos que não fossem múltiplos do período de amostragem, esse erro existiria. Trata-se, portanto, de condições muito favoráveis, e não de um desempenho representativo — o que motivou as duas baterias seguintes, conduzidas sobre o banco de estímulos externo.

\subsection{Baterias com o banco de estímulos externo}
\label{sec:resBateriaReal}

A quarta bateria empregou os seis arquivos do banco de estímulos externo, cedido para este trabalho --- sinais sintéticos, de complexo P-QRS-T invariante ao longo do registro ---, cujos ritmos situam-se próximos aos limiares de decisão, e resultou em 60,6\,\% de taxa de acerto total. A bradicardia caiu a 3,3\,\%, o que confirmou, agora de forma direta, a hipótese da descontinuidade entre segmentos: os arquivos usados têm exatamente 30~s e não contêm um número inteiro de batimentos, de modo que surge um intervalo R-R falso na junção fim--início dos segmentos gerados pelo gerador de sinais, presente em todos eles, o que desloca o intervalo R-R médio para 1000~ms --- exatamente o limiar de bradicardia. Ajustando-se apenas o trecho de linha de base entre o último batimento e o fim de cada arquivo, sem alterar a morfologia dos batimentos, o intervalo da junção passou a coincidir com o intervalo mediano e a sensibilidade da bradicardia subiu para 100\,\% (bateria 4b, com taxa de acerto total de 65,0\,\%). Essa bateria revelou ainda que os estímulos de normal e de arritmia do banco possuem frequência cardíaca de exatamente 60~bpm, sobre o limiar de bradicardia, o que dificulta a detecção: os erros de temporização, normais em qualquer cadeia de aquisição, eventualmente farão alguns segmentos normais serem classificados como bradicardia. Trata-se de limitação do material de teste, e não do detector.

A quinta bateria foi realizada para eliminar os problemas observados nas baterias anteriores. Gerou-se um sinal sintético que adota como molde o próprio batimento do banco externo, sob ritmo controlado e distante dos limiares de decisão: intervalo R-R de 800~ms (75~bpm) no traçado normal, 500~ms (120~bpm) na taquicardia e 1.200~ms (50~bpm) na bradicardia, com variabilidade batimento a batimento de desvio-padrão de 15, 10 e 20~ms, respectivamente --- valores bem abaixo do limiar de 120~ms do critério de arritmia sinusal, para não induzir classe falsa. Sobre esse ritmo somaram-se imperfeições de amplitude, todas referidas à amplitude da onda R: deriva senoidal de linha de base de 5\,\%, modulação respiratória de $\pm$8\,\% e ruído branco de desvio-padrão de 2\,\%. A modulação respiratória foi inserida para aproximar o estímulo de um \acs{ECG} real, já que não havia sido utilizada nas baterias anteriores. Os arquivos foram gerados com número inteiro de ciclos, de modo que a junção entre os segmentos de 30~s produzidos pelo gerador fica sem descontinuidade.

\subsection{Resultado da bateria final}
\label{sec:resBateriaFinal}

A Figura~\ref{fig:casos6classes} ilustra, para cada uma das seis classes, um trecho representativo do estímulo empregado nesta bateria, evidenciando as características de ritmo que fundamentam a classificação: a regularidade do traçado normal, o encurtamento e o alargamento dos intervalos na taquicardia e na bradicardia, a variabilidade batimento a batimento da arritmia sinusal, e as pausas do bloqueio e da parada sinoatrial. A distinção entre estas duas últimas não é de duração absoluta, mas de razão com o ritmo de base, conforme o critério da Seção~\ref{sec:deteccaoBorda}: a pausa do bloqueio equivale a três intervalos R-R medianos, isto é, à ausência de dois complexos QRS, após a qual o ritmo é retomado; a da parada equivale a três intervalos e meio, excedendo três e não correspondendo a múltiplo inteiro do ritmo de base.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{assets/casos_6classes.pdf}
\caption{Exemplos das seis classes de estímulo empregadas na validação final, reproduzidos pelo gerador NI myDAQ e apresentados na escala de amplitude medida na entrada do \acs{AFE}. No painel de bloqueio a janela é centrada na pausa --- de 3,00 vezes o intervalo R-R mediano, equivalente à ausência de dois complexos QRS ---, de modo que se observe a retomada do ritmo; no de parada, a janela mostra primeiro os batimentos e termina dentro da pausa, de 3,50 vezes o intervalo mediano.}
...
\begin{table}[htb]
\centering
\caption{Taxa de acerto sobre os 60 segmentos do MIT-BIH nas três configurações do critério de detecção.}
...
Enquanto a base MIT-BIH forneceu \acs{ECG} real anotado para medir a generalização do detector, o banco de estímulos do orientador — empregado desde a quarta bateria — permite uma verificação complementar, centrada na relação entre o ritmo de cada estímulo e os limiares de decisão. A quarta bateria, que empregou o \acs{ECG} sintético desse banco, atingira apenas 60,6\,\% (Seção~\ref{sec:resBateriaReal}), em boa parte por conta da descontinuidade entre segmentos, artefato próprio da reprodução em laço. Adotado o protocolo de disparo único, que reproduz cada arquivo uma única vez e alinha o segmento medido ao início da reprodução, esses mesmos seis arquivos foram reavaliados na taxa em que foram criados (500~amostras por segundo), na sétima bateria. A taxa de acerto subiu a 77,8\,\% (Tabela~\ref{tab:matrizProfessor}), com quatro das seis classes reconhecidas integralmente — o que confirma, por contraste com a quarta bateria, que boa parte de seu erro era artefato de bancada, e não do detector.
...
As 40 falhas restantes concentram-se em dois arquivos, e a análise de ambos mostra tratar-se de defeito do estímulo, e não do detector. O arquivo de arritmia sinusal apresenta intervalo R-R médio de 1004,6~ms, acima do limiar de bradicardia (1000~ms); em consequência, todo segmento satisfaz simultaneamente os critérios de arritmia e de bradicardia, e a regra de precedência atribui a classificação à bradicardia. O critério de arritmia, note-se, foi satisfeito nos 30 segmentos — a detecção da variabilidade ocorreu; o que falha é a classificação exclusiva, porque o ritmo médio do estímulo recai sobre a fronteira do limiar. O arquivo de bloqueio sinoatrial, por sua vez, contém apenas duas pausas na faixa de 4 a 6~s exigida pelo critério, uma delas exatamente sobre a borda de 4~s; a quantização ocasionalmente a desloca para fora da faixa, reduzindo a contagem de pausas ao mínimo e deixando 10 dos 30 segmentos sem classificação.
...
A Figura~\ref{fig:ruidoTipos} ilustra os três tipos de perturbação, na maior amplitude, somados ao estímulo normal. A Tabela~\ref{tab:ruidoAcuracia} resume a taxa de acerto por condição. A taxa de acerto total foi de 93,5\,\% (1.515 dos 1.620 ensaios).
...
A décima terceira bateria, conduzida sobre a configuração assim confirmada, classificou corretamente os 180 segmentos (Tabela~\ref{tab:matrizBat13}). Cada estímulo acendeu exclusivamente a sua própria \textit{flag}, e em nenhum dos 180 segmentos a sexta classe foi ativada. Esta segunda observação tem peso equivalente ao da taxa de acerto: uma classe destinada ao que não se enquadra, se disparasse sobre sinal limpo de classe conhecida, constituiria defeito, não cobertura.
...
A identidade dos resultados é o desfecho desejável, e não um resultado nulo: demonstra que a sexta classe cobre as lacunas identificadas na Seção~\ref{sec:resZonaMorta} \emph{sem} perturbar o comportamento já validado. O ganho que ela produz manifesta-se onde as lacunas existem --- na varredura de durações de pausa e sobre a base MIT-BIH, cuja taxa de acerto passa de 71,7\,\% para 85,0\,\% (Tabela~\ref{tab:mitbihConfig}) ---, e não sob perturbações que não as exercitam.
